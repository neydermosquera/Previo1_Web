...
if (editando == false) {
        ...
        var nuevoCodigoHtml = '<td><input class="form-control" type="text" name="alimento" id="alimento" value="'+alimento+'" size="10"></td>'+
                            ...
                            '<td><select id="validadIcono" class="form-control"><option id="bien" value="1">Bien</option><option id="mal" value="2">Mal</option></select>'+ '<td>En edición</td>';
...
function capturarEnvio() {
        var nodoContenedorForm = document.getElementById('contenedorForm'); 
        nodoContenedorForm.innerHTML = '<form name = "formulario" action="resivido.html" method="get" onsubmit="capturarEnvio()" onreset="anular()">'+
        '<input type="text" name="alimento" value="'+document.querySelector('#alimento').value+'">'+
